...
gulp.task('publish-static-js',['date-picker'],function(){
    return gulp.src([path.join(__dirname, '/dep/vue.js')])
        .pipe(gulp.dest(path.join(__dirname, '/bundle/js/')));
});

gulp.task('date-picker',['less-to-css'],function(){
    return gulp.src([path.join(__dirname, '/dep/My97DatePicker/**/*.*')])
        .pipe(gulp.dest(path.join(__dirname, '/bundle/js/My97DatePicker')));
});
...
gulp.task('rev-html', ['dist-img'], function(){
    return gulp.src([path.join(__dirname, '/mfg/rev/*.json'), path.join(__dirname, '/Areas/*/Views/*/*.cshtml')])
        .pipe(revCollector({
            replaceReved: true
        }))
        .pipe(gulp.dest(path.join(__dirname, '/Areas/')))
});
...
gulp.task('less', ['del-dist'], function () {
    return gulp.src(path.join(__dirname, '/less/**/*.less'))
